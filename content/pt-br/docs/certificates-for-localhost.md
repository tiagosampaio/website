...
Às vezes algumas pessoas desejam obter um certificado para o hostname "localhost",
seja para uso no desenvolvimento local ou para distribuição com um aplicativo nativo que
precisa se comunicar com um aplicativo na web. A Let's Encrypt não pode fornecer
certificados para "localhost" porque ninguém o possui exclusivamente e não é
enraizada em um domínio de nível superior como ".com" ou ".net". É possível
configurar seu próprio nome de domínio que será resolvido para `127.0.0.1` e obter um
certificado para ele usando o desafio de DNS. No entanto, isso geralmente é uma má
idéia e existem opções melhores.
...
# Para apps nativos se comunicando com apps web
...
# Criando e confiando em seus próprios certificados
